...
def interact_logistic_profitable_orders_dict(profitable_orders_dict: dict):
    ...
    while True:
        ...
        for page_num in range(page_cursor, end_num):
            ...
            print("[{}/{}]{:>6d}\tprofit: {:>15,.1f}\tcost: {:>15,.2f}\tprofit_rate: {:>5.1f}\tvolume: {:<7,}\t"
                  "buyer: {:>2d} seller: {:>2d} rating: {:<3.2f}\t{}"
                    ...
                    while True:
                        print("{} {} volume: {}\tprofit_rate: {:.2f}\tprofit: {:,.2f}\tcost: {:,.2f}\tbuyer: {}\t"
                              ...
                              .format(int(il_choice_1), tmp_dict['type_name'], tmp_dict['volume'],
                                      ...
                                      len(tmp_dict['buy']), len(tmp_dict['sell'])))
                        print('''---------------------
enter) 全部显示
b) 显示买单详情   
s) 显示卖单详情
...
''')
                        ...
                        if il_choice_2 == '':
                            print("{} {} volume: {}\tprofit: {:,.2f}\tprofit_rate: {:.2f}\tcost: {:,.2f}\tbuyer: {}\t"
                                  "seller: {}"
                                  .format(int(il_choice_1), tmp_dict['type_name'], tmp_dict['volume'],
                                          tmp_dict['profit'],
...
def interact_orders_dicts_list(order_dicts_list: list, order_dicts_list_2=None):
    ...
    # type_id = o_list[0]['typeid']
    # type_name = data_helper.get_type_name(typeid)
    ...
    #         order = o_list[num]
    #         display_order(order, num, list_len)
    #
    #     io_choice_1 = input("(w:prev, z:next, g:top, G:end, q:back):")
    #     if io_choice_1 == 'q':
    #         return
